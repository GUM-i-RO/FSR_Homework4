...
gamma_min = 0; 
gamma_max = alpha;
gamma_range = linspace(gamma_min, gamma_max, num_initial_conditions);

omega1_range = sqrt(2*g/l*(1-cos(gamma_range-alpha)));

h = figure('Renderer', 'painters', 'Position', [10 10 900 250]);
plot(gamma_range, omega1_range, 'k-', 'Linewidth', lw ,'Color', [0.2, 0.2, 0.2], 'HandleVisibility', 'off');
hold on;
plot([0 gamma_range(end)], [0.95 0.95], 'k--', 'LineWidth',lw,'Color', [0.5, 0.5, 0.5]);
plot([alpha alpha], [0 1], 'k--', 'LineWidth',lw,'Color', 'r');
legend('$\dot{\theta}_0$', '$\alpha$');
...
%axis equal
axis padded
xticks(0:0.05:gamma_range(end));  % More ticks
xlim([0 gamma_range(end)])
%set(gca, 'InnerPosition', [0.1400 0.32 0.82 0.55])
%annotation('rectangle',[0 0 1 1],'Color','w');
%title('Heading Velocity profile unscaled')
%exportgraphics(h, '4omega1_gamma.pdf');
