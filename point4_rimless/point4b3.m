...
for k = 1:2
    ...
    if k == 2
        plot(res(4).T, res(4).Y(:,2), 'k-', 'LineWidth', lw, 'Color', [0.2, 0.2, 0.2]);
        plot(res(3).T, res(3).Y(:,2), 'k-', 'LineWidth', lw, 'Color', 'r');
        plot(res(2).T, res(2).Y(:,2), 'k-', 'LineWidth', lw, 'Color', 'b');
        plot(res(1).T, res(1).Y(:,2), 'k-', 'LineWidth', lw, 'Color', [0.8, 0.0, 0.8]);
        xlabel('$t$ [s]', 'Interpreter', 'latex');
        %plot([T(1) T(end)], [gamma-alpha gamma-alpha], 'k--', 'LineWidth',lw,'Color', [0.5, 0.5, 0.5]);
        %plot([T(1) T(end)], [gamma+alpha gamma+alpha], 'k--', 'LineWidth',lw,'Color', [0.5, 0.5, 0.5]);
        legend('0.24','0.16','0.08','0');
        legend('Location','south','Orientation','horizontal','AutoUpdate','off')
    end
    if k == 1
        plot(res(4).T, res(4).Y(:,1), 'k-', 'LineWidth', lw, 'Color', [0.2, 0.2, 0.2]);
        plot(res(3).T, res(3).Y(:,1), 'k-', 'LineWidth', lw, 'Color', 'r');
        plot(res(2).T, res(2).Y(:,1), 'k-', 'LineWidth', lw, 'Color', 'b');
...
set(h, 'MenuBar', 'none');
set(h, 'ToolBar', 'none');
%exportgraphics(h, ['4theta&dot.pdf']);
...
hold on
plot(res(3).Y(:,1), res(3).Y(:,2), 'k-', 'Linewidth', lw ,'Color', 'r');
plot(res(2).Y(:,1), res(2).Y(:,2), 'k-', 'Linewidth', lw ,'Color', 'b');
plot(res(1).Y(:,1), res(1).Y(:,2), 'k-', 'Linewidth', lw ,'Color', [0.8, 0.0, 0.8]);
plot([gamma_value(1)-alpha gamma_value(1)-alpha], [-1 2.7], 'k--', 'Linewidth', lw ,'Color', [0.5, 0.5, 0.5]);
plot([gamma_value(1)+alpha gamma_value(1)+alpha], [-1 2.7], 'k--', 'Linewidth', lw ,'Color', [0.5, 0.5, 0.5]);
plot([gamma_value(2)-alpha gamma_value(2)-alpha], [-1 2.7], 'k--', 'Linewidth', lw ,'Color', [0.5, 0.5, 0.5]);
plot([gamma_value(2)+alpha gamma_value(2)+alpha], [-1 2.7], 'k--', 'Linewidth', lw ,'Color', [0.5, 0.5, 0.5]);
plot([gamma_value(3)-alpha gamma_value(3)-alpha], [-1 2.7], 'k--', 'Linewidth', lw ,'Color', [0.5, 0.5, 0.5]);
plot([gamma_value(3)+alpha gamma_value(3)+alpha], [-1 2.7], 'k--', 'Linewidth', lw ,'Color', [0.5, 0.5, 0.5]);
plot([gamma_value(4)-alpha gamma_value(4)-alpha], [-1 2.7], 'k--', 'Linewidth', lw ,'Color', [0.5, 0.5, 0.5]);
plot([gamma_value(4)+alpha gamma_value(4)+alpha], [-1 2.7], 'k--', 'Linewidth', lw ,'Color', [0.5, 0.5, 0.5]);
plot(res(4).Y(1,1), res(4).Y(1,2),'Color', [0.2, 0.2, 0.2], 'Marker','*','LineWidth',5)
plot(res(3).Y(1,1), res(3).Y(1,2),'Color', 'r', 'Marker','*','LineWidth',5)
plot(res(2).Y(1,1), res(2).Y(1,2),'Color', 'b', 'Marker','*','LineWidth',5)
